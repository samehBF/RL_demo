...
# Inspired from Carlos Aguayo algorithm
...
def cart_pole_with_qlearning():
    ...
    cart_position_bins = pd.cut([-2.4, 2.4], bins=4, retbins=True)[1][1:-1]
    pole_angle_bins = pd.cut([-2, 2], bins=4, retbins=True)[1][1:-1]
    cart_velocity_bins = pd.cut([-1, 1], bins=4, retbins=True)[1][1:-1]
    angle_rate_bins = pd.cut([-3.5, 3.5], bins=4, retbins=True)[1][1:-1]
